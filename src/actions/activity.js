...
export const createActivity = (data) => async (dispatch) => {
  try {
    const response = await axios({
      method: 'POST',
      url: `https://final-api-backend.herokuapp.com/users/${data.user_id}/activities`,
      data,
      crossdomain: true,
...
export const deleteActivity = (data) => async (dispatch) => {
  try {
    ...
    const response = await axios({
      method: 'DELETE',
      url: `https://final-api-backend.herokuapp.com/users/${data.user_id}/activities/${data.id}`,
      data,
      crossdomain: true,
      withCredentials: true,
    });
